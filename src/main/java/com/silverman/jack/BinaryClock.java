...
//		gpio = GpioFactory.getInstance();
	...
	private StringBuilder convertToBinary() {
		...
		for (int i = 0; i < splitStringTime.length; i++) {
			binaryTime.append(String.format("%" + 6 + "s", Integer.toBinaryString(Integer.parseInt(splitStringTime[i])))
					.replace(" ", "0"));
//			String.format(, Integer.toBinaryString(i)).replace(' ', '0')
		}
		System.out.println(binaryTime.toString());
		return binaryTime;
	}
